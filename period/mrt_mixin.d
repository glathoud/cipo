...
*/
{
  ...
  */
  ...
  static string code_one_a( in uint i, in ulong a ) pure
  {
    string _sa = to!string( a );
    string block_id = "one_block_" ~ to!string( i ) ~ "_" ~ _sa;
    
    return block_id ~ `: while( true ) {
      ...
      if (t == 1 || t == nm1) break ` ~ block_id ~ `; // n ist probably prime

      for (int j2=1 ; j2<j ; ++j2) {
        t = t*t % n;

        if (t == nm1) break ` ~ block_id ~ `;
        if (t <= 1) break;
      }
  ...
  static string code( in uint i_case, in ulong[] det_arr ) pure
  {
    uint i = i_case * 100;
    return `{
      ` ~ det_arr.map!( a => code_one_a( i++, a ) ).join( "\n" ) ~ `
  ...
  if (n < 25_326_001UL)
    {
      if (n < 2_047UL)
        mixin( code( 0, det_arr_0 ) );

      else if (n < 1_373_653UL)
        mixin( code( 1, det_arr_1 ) );

      else if (n < 9_080_191UL)
        mixin( code( 2, det_arr_2 ) );

      else
        mixin( code( 3, det_arr_3 ) );
    }
  else if (n < 2_152_302_898_747UL)
    {
      if (n < 3_215_031_751UL)
        mixin( code( 4, det_arr_4 ) );

      else if (n < 4_759_123_141UL)
        mixin( code( 5, det_arr_5 ) );

      else if (n < 1_122_004_669_633UL)
        mixin( code( 6, det_arr_6 ) );

      else
        mixin( code( 7, det_arr_7 ) );
    }
  else {

    if (n < 3_474_749_660_383UL)
      mixin( code( 8, det_arr_8 ) );
        
    else if (n < 341_550_071_728_321UL)
      mixin( code( 9, det_arr_9 ) );
        
    else if (n < 3_825_123_056_546_413_051UL)
      mixin( code( 10, det_arr_10 ) );

    else
      mixin( code( 11, det_arr_11 ) );
  }
